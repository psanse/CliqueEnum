...
virtual	void print_edges		(std::ostream& = cout)			const;
virtual	void print_edges		(T& bbsg, std::ostream& = cout)	const;
virtual	void print_data			(bool lazy=true, std::ostream& = cout);
...
template<class T>
inline
void Ugraph<T>::print_edges (T& bbsg, std::ostream& o) const{
	for(int i=0; i<Graph<T>::m_size-1; i++){
...
// writes file in dimacs format with timestamp
	...
	if (Graph<T>::is_weighted_v()){
		for(int v=0; v<Graph<T>::m_size; v++){
			o<<"n "<<v+1<<" "<<Graph<T>::get_wv(v)<<endl;
		}
	}
...
void Ugraph<T>::print_data(bool lazy, std::ostream& o) {
	o<<Graph<T>::number_of_vertices()<<" "<<fixed<<number_of_edges(lazy)<<" "<<std::setprecision(6)<<density()<<" "<<Graph<T>::m_name.c_str();
	if(Graph<T>::m_is_wv) o<<" w";
	o<<endl;
}
...
// (applied as pivot strategy for enumeration)
	...
	int ndeg=0;
	int nBB=WDIV(v);
	for(int i=nBB+1; i<Graph<T>::m_BB;i++)
		ndeg+=bitblock::popc64(Graph<T>::m_g[v].get_bitboard(i)& bbn.get_bitboard(i));
	//truncate the bitblock of v
	ndeg+=bitblock::popc64(bitblock::MASK_1(WMOD(v)+1,63) & Graph<T>::m_g[v].get_bitboard(nBB)& bbn.get_bitboard(nBB));
return ndeg;
}
