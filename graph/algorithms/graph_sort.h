...
////////////////
namespace gbbs{
	enum sort_t						{MIN_DEG_DEGEN=0, MAX_DEG_DEGEN, MIN_DEG_DEGEN_TIE_STATIC, MAX_DEG_DEGEN_TIE_STATIC, MAX_DEG_DEGEN_TIE, KCORE, KCORE_UB, MAX_WEIGHT, MIN_WEIGHT, MAX_WEIGHT_DEG, MIN_WEIGHT_DEG, MAX_DEG_ABS, NONE};
...
//computes a reordering [OLD_INDEX]=NEW_INDEX
	...
	int change_order				(const typename Graph_t::bb_type& sgfrom, const typename Graph_t::bb_type& sgref, vint& ord, gbbs::pick_t, gbbs::place_t, bool is_degen);		
private:
...
// 3-Assumes Decode state is properly updated before the call
	...
	//copy back t the original graph
	g=gn;
...
// 2.There was a lot to do! Basically degrees with respect to the vertex removed and support can be recomputed over the updated degrees.
	...
	//computes order
	typename Graph_t::bb_type bbn(NV);
	bbn.set_bit(0,NV-1);
	switch(alg){
	case gbbs::MIN_DEG_DEGEN:
		while(!degs.empty()){
...
template<typename Graph_t>
int GraphSort<Graph_t>::change_order (const typename Graph_t::bb_type& sgfrom_in, const typename Graph_t::bb_type& sgref_in, vint& ord, gbbs::pick_t pick, gbbs::place_t place, bool is_degen){
///////////////////////
